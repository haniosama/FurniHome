...
const ProductDashboard = () => {
    ...
    // const [updataProduct,setUpdataProduct]=useState<IProduct>();
    ...
    const {products,specificProduct,isLoading}:{products:IProduct[],isLoading:boolean,specificProduct:IProduct[]}=useSelector((state:IState)=>state.dashBoard);
    ...
    const handleUpdataProduct=useCallback(async(productId?:string,formData?:FormData)=>{
        setOpenUpdataProductContainer(true);
        console.log(productId)
        if(formData){
            await dispatch(updataProduct(formData))
        }
        if(productId){
    ...
    return (
        ...
        {openUpdataProductContainer
            &&
            <UpdataProductForm specificProduct={specificProduct[0]} handleUpdataProduct={handleUpdataProduct} setOpenUpdataProductContainer={setOpenUpdataProductContainer}/>
        }
                        ...
                        {(searchProducts || products) ? (
                            (searchProducts || products).map((item) => (
                                ...
                                <td className="px-4 py-6 text-center flex gap-1">
                                    <button onClick={()=>handleUpdataProduct(item._id)} className="bg-sky-700 px-2 py-1 rounded-lg text-white cursor-pointer border border-blue-700 transition-all duration-300 hover:bg-white hover:text-blue-700">
                                    Updata
                                    </button>
