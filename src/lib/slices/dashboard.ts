...
export const getCustomerForAdmin=createAsyncThunk('Admin_Producs/getCustomer',async()=>{
    const data =await (await fetch(`${baseUrl}/api/customer/${user?.userID}`,{
        headers:{
...
const productForAdmin=createSlice({
    name:"Admin_Producs",
    initialState:{products:[] as Iproduct[], customers:[] as [], isLoading:false as boolean, error:null as string | null},
    reducers:{},
